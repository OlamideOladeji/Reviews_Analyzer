...
thetas=p1.average_perceptron(train_bow_features, train_labels, 15)
avg_pct_train_accuracy, avg_pct_test_accuracy = \
   p1.average_perceptron_accuracy(train_bow_features,test_bow_features,train_labels,test_labels,T=15)
print("{:43} {:.4f}".format("Training set accuracy for average perceptron:", avg_pct_train_accuracy))
print("{:43} {:.4f}".format("Testing accuracy for average perceptron:", avg_pct_test_accuracy))
...
#-------------------------------------------------------------------------------
(theta,theta0)=p1.average_perceptron(train_bow_features, train_labels, 15)
best_theta = theta
...
#-------------------------------------------------------------------------------
dictionary = p1.bag_of_words_with_bigrams(train_texts)
#
...
#L = 0.01
#
(theta,theta0)=p1.average_perceptron(train_final_features, train_labels, 15)
best_theta = theta
wordlist   = [word for (idx, word) in sorted(zip(dictionary.values(), dictionary.keys()))]
sorted_word_features = utils.most_explanatory_word(best_theta, wordlist)
print("Most Explanatory Word Features")
...
#Code to  check and tune avgperceptron based on new bag of words
avg_pct_train_accuracy, avg_pct_val_accuracy = \
   p1.average_perceptron_accuracy(train_final_features,val_final_features,train_labels,val_labels,T=T)
print("{:43} {:.4f}".format("Training accuracy for average perceptron for final features:", avg_pct_train_accuracy))
print("{:43} {:.4f}".format("Validation accuracy for average perceptron for final features:", avg_pct_val_accuracy))
...
dictionary = p1.bag_of_words_with_bigrams(submit_texts)
train_final_features = p1.extract_final_features(train_texts, dictionary)
submit_final_features = p1.extract_final_features(submit_texts, dictionary)
#
 # 2. Train your most accurate classifier
final_thetas = p1.average_perceptron(train_final_features, train_labels, T=15)
#
